...
class GameOfLife {
    ...
    GameOfLife(String[] rows, Counter counter) {
        int colCnt = rows[0].replace(" ", "").length();
        int rowCnt = rows.length;
        board = new Board(new CoordRow(rowCnt), new CoordCol(colCnt));
        for (int r = 0; r < rowCnt; r++) {
            String row = rows[r].replace(" ", "");
            for (int c = 0; c < colCnt; c++) {
                if(row.charAt(c)=='X') board.getAt(new CoordRow(r), new CoordCol(c)).set();
            }
        }
        this.counter = counter;
    }

    String state() { return board.getText(); }
    ...
    void populateGlider(CoordRow row, CoordCol col) {
...
class Counter {
    private int value;
    Counter() { value = 0; }
    Counter(String counter) { value = Integer.parseInt(counter); }
...
class Cell {
    ...
    private final CoordRow row;
    private final CoordCol col;
...
class Coord {
    ...
    boolean isBetween(Coord low, Coord high) { return low.value >= value && value < high.value; }
...
class CoordRow extends Coord {
    ...
    CoordRow(Coord coord) { this(coord.get()); }
...
class CoordCol extends Coord {
    ...
    CoordCol(Coord coord) { this(coord.get()); }
...
class Board {
    ...
    private final ArrayList<Coords> neighbourhood;
    ...
    String getText() {
        return board.stream().reduce("", (t, cell) -> t + cell.toString() + ((cell.col().get()==COLS.get()-1) ? "\n" : ""), String::concat);
    }
    ...
    void cycle() {
        ...
        board.forEach(Cell::nextGeneration);
    }
